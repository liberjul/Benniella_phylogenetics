...
#SBATCH --job-name=assemble_ben_reads_NVP103_pe        # you can give your job a name for easier identification (same as -J)
...
do
  ...
  if [ $(echo $i | cut -d'/' -f1) == "BenspLightDraft_FD_NVP103" ]
  then
    ...
    mkdir $OUTDIR/$BASE/
    cp $WORKDIR/spades_$BASE/*.fasta $OUTDIR/$BASE/
  fi
done
conda deactivate
